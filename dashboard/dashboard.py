...
min_date = dataFrame["datetime"].min().date()  # tanggal minimum
max_date = dataFrame["datetime"].max().date()  # tanggal maksimum
...
st.markdown("""
            ...
            ### Gambaran Proyek
            Dashboard ini menyajikan analisis data kualitas udara, dengan perhatian khusus pada tingkat PM2.5 dari beberapa stasiun. Proyek ini bertujuan untuk mengungkap tren, variasi musiman, dan dampak berbagai kondisi cuaca terhadap kualitas udara. Wawasan dari analisis ini dapat bermanfaat bagi studi lingkungan dan pemantauan kesehatan masyarakat.
            """
            )

# Pertanyaan 1
st.subheader("Variasi Kualitas Udara Berdasarkan Musim (PM2.5, PM10)".capitalize())
...
# Pertanyaan 2
st.subheader("pola sebaran polutan berbahaya (SO2, NO2, CO) di berbagai musim dan waktu dalam sehari".capitalize())
...
# Pertanyaan 3
st.subheader("Perubahan tren kualitas udara (PM2.5, PM10) selama periode 2013 hingga 2017".capitalize())
...
# Pertanyaan 4
st.subheader("Korelasi antara curah hujan (RAIN) dengan tingkat polusi udara (PM2.5, PM10)".capitalize())
